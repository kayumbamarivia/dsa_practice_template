...
public:
    ...
    void loadData()
    {
        loadVehicleList(lotId + "_vehicles.csv", vehicles);
        loadSpotList(lotId + "_spots.csv", spots);
        loadSessionList(lotId + "_sessions.csv", sessions);
    }
...
private:
    ...
    void loadVehicleList(const string &fn, Vehicle *&head)
    {
        ifstream f(fn);
        if (!f) return;
        string line;
        ...
        while (getline(f, line))
        {
            ...
            string lp, type, owner;
            getline(ss, lp, ',');
            getline(ss, type, ',');
            getline(ss, owner, ',');
            head = new Vehicle{lp, type, owner, head};
        }
    }

    void loadSpotList(const string &fn, ParkingSpot *&head)
    {
        ifstream f(fn);
        if (!f) return;
        string line;
        ...
        while (getline(f, line))
        {
            ...
            getline(ss, type, ',');
            getline(ss, occ, ',');
            head = new ParkingSpot{stoi(id), type, occ == "1", head};
        }
    }

    void loadSessionList(const string &fn, ParkingSession *&head)
    {
        ifstream f(fn);
        if (!f) return;
        string line;
        ...
        {
            stringstream ss(line);
            string id, vId, sId, entry, exit;
            getline(ss, id, ',');
            getline(ss, vId, ',');
            getline(ss, sId, ',');
            getline(ss, entry, ',');
            getline(ss, exit, ',');
            head = new ParkingSession{stoi(id), vId, stoi(sId), entry, exit, head};
        }
    }
